  ...
  void parseAndCheckGt(std::ifstream& gtfile)
  {
    ...
    if (gtfile.good())
    {
      ...
      for (const auto& it : data)
      {
        ...
        gt.timestamp_ = it.at(groundtruth_indices.at(0)) > 10e1 ? it.at(groundtruth_indices.at(0)) / 1e9 :
  ...
  void parseAndCheckImu(std::ifstream& imufile)
  {
    ...
    if (imufile.good())
    {
      ...
      for (const auto& it : data)
      {
        ...
        imu.timestamp_ = it.at(imu_indices.at(0)) > 10e1 ? it.at(imu_indices.at(0)) / 1e9 : it.at(imu_indices.at(0));
  ...
  void parseAndCheckImages(std::ifstream& imgfile)
  {
    ...
    if (imgfile.good())
    {
      ...
      for (const auto& it : data)
      {
        ...
        cam.timestamp_ = std::stod(it.at(image_indices.at(0))) > 10e1 ? std::stod(it.at(image_indices.at(0))) / 1e9 :
                                                                         std::stod(it.at(image_indices.at(0)));
        cam.timestamp_ += timeoffset_;

        cam.image_ = cv::imread(image_data_folder_ + it.at(image_indices.at(1)), cv::IMREAD_GRAYSCALE);
